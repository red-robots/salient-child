  ...
  <div class="row" id="copyright" data-layout="<?php echo esc_attr( $copyright_footer_layout ); ?>">
	...
	?>
			...
			<p>&copy; <?php echo date( 'Y' ) . ' ' . esc_html( get_bloginfo( 'name' ) ); ?>. 
			 	...
			 	<?php
			 	$footerLinks = get_field("footer_link","option"); 
			 	if($footerLinks) { ?>
				<span id="additionFootLinks">
					<?php foreach ($footerLinks as $flink) { ?>
						<?php if ( $flink['name'] && $flink['link'] ) { ?>
						<a href="<?php echo $flink['link'] ?>"><?php echo $flink['name'] ?></a>
						<?php } ?>
					<?php } ?>
				<?php } ?>
			 </p>
