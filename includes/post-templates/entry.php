...
$page_class = ($post_type=='post') ? ' blogpage':'';
...
<article id="post-<?php the_ID(); ?>" <?php post_class( $nectar_post_class_additions.$page_class); ?>>
		...
		<div class="post-content">
  		 ...
  		 */
			if ( $using_masonry != true && $blog_standard_type == 'featured_img_left' ) {
				...
				if ( ! is_single() ) {
					$div_class = ($post_type=='post') ? ' blog-item':'';
					?>
				  

					 <div class="article-content-wrap<?php echo $div_class ?>">
						...
						<div class="post-featured-img-wrap"> 
							...
							if( $post_type=='post' ) {
								$id = get_the_ID();
								$placeholder = get_stylesheet_directory_uri() . '/images/rectangle-lg.png';
								$thumbID = get_post_thumbnail_id($id);
								$img = wp_get_attachment_image_src($thumbID,'large');
								$style = ($img) ? ' style="background-image:url('.$img[0].')"':'';
								?>

								<div class="image <?php echo ($img) ? 'has-image':'no-image' ?>"<?php echo $style ?>>
